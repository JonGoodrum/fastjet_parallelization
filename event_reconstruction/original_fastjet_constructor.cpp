...
//   g++ original_fastjet_constructor.cpp -o OFC  `fastjet-install/bin/fastjet-config --cxxflags --libs --plugins` -pthread -std=c++11
//  To run, you can execute the following:
//   sudo ./OFC < some_input_event.dat
...
void printjets(std::vector<PseudoJet> jets , bool append){
  ...
  if(!append)
    {PrintToFile.open ("ofc_results.txt");}
  else
    {PrintToFile.open ("ofc_results.txt", std::ios::app|std::ios::ate);}
